    ...
    match choice:
        ...
        case 12:
            while True:
                ...
                purpose = input(
                    u"\U0001F539"+"Specify your purpose of accessing refugee's request system? (add or edit or clear): ")
                if purpose != "add" and purpose != "edit" and purpose != "clear":
                    ...
                    if purpose != "clear":
                        if request == "0" and purpose == "edit":
                            ...
                            cont = yn_valid(
                                u"\U0001F539"+"Would you like to abort your request edition? (Yes/No): ")
                            if cont == "Yes":
                        ...
                        if df_task_by_ref.empty:
                            warn(
                                "The request is already empty. There is nothing to clear out.")
                            cont = yn_valid(
                                u"\U0001F539"+"Would you like to continue with request edition? (Yes/No): ")
                            if cont == "No":
                                ref.ref_row.append(0)
                                break
                        else:
                            print("Please see the task schedule below: \n")
                            print_table(df_task_by_ref.columns, df_task_by_ref.to_numpy(
                            ).tolist(), (18, 22, 30, 40, 14, 40, 40, 30))
                            # volunteer schedule clear
                            clear_request_schedule(conn, df_task_by_ref)
                            # refugee
                            update_refdb_attr(conn, refugeeID, "request", "0")
                            print(
                                u"\U0001F539"+"The request schedule related to volunteer and refugee are successfully cleared out.")
                            ref.ref_row.append(0)
                            break
        case 13:
            return 0
